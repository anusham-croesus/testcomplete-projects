...
function PreExecution()
{
    ...
    if (aqFileSystem.GetFileNameWithoutExtension(ProjectSuite.FileName) != "ProjectSuiteMiniRegression" || GetIndexOfItemInArray(["PreExecution", (GetProjectSuiteEnabledProjectsNamesList()[0])], aqFileSystem.GetFileNameWithoutExtension(Project.FileName)) != -1){
        DisableDoubleAuthentication(vServerRelations, false);//Désactiver la double authentification
    ...
    RestartServicesAndCheckVServer(vServerRelations); //Par défaut, toute l'exécution s'arrête en cas d'échec de la vérification du VServer
    //UpdateClientVserverAssembleScript(vServerRelations, true); //Désactivée car la MAJ de l'Assemble Script est faite par Jenkins
}