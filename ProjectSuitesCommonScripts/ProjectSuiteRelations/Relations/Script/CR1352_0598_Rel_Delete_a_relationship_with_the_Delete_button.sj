...
/**
    Description : Supprimer une relation par le bouton "-"
    https://jira.croesus.com/testlink/linkto.php?tprojectPrefix=Croes&item=testcase&id=Croes-598
    Analyste d'assurance qualité : Reda Alfaiz
    Analyste d'automatisation : Christophe Paring
*/

function CR1352_0598_Rel_Delete_a_relationship_with_the_Delete_button()
{
...
//        var randomIndex = Math.round(Math.random()*(arrayOfRelationships.length - 1))
        randomRelationship = arrayOfRelationships[0];
...
/********************************************CR1352_0599_Rel_Delete_a_relationship_with_the_mouse_right_click***********************************************************************/       
  //Choisir aléatoirement une relation à supprimer
//        var randomIndex = Math.round(Math.random()*(arrayOfRelationships.length - 2))
        randomRelationship = arrayOfRelationships[1];
        
        //Sélectionner la relation à supprimer, faire clic droit puis Supprimer
        Log.Message("Select a random relationship : '" + randomRelationship + "' and click on the Delete button.");
        Get_ModulesBar_BtnRelationships().Click();
        Get_ModulesBar_BtnRelationships().WaitProperty("IsChecked", true, 100000);
        //Get_Toolbar_BtnRedisplayAllAndRemoveCheckmarks().Click();
        //Delay(2000);
        SearchRelationshipByName(randomRelationship);
        Get_RelationshipsClientsAccountsGrid().FindChild("Value", randomRelationship, 10).ClickR();
...
/********************************************CR1352_0600_Rel_Delete_a_relationship_with_the_Edit_menu***********************************************************************/       
    //Choisir aléatoirement une relation à supprimer
//        var randomIndex = Math.round(Math.random()*(arrayOfRelationships.length - 3))
        randomRelationship = arrayOfRelationships[2];
        ...
        RestoreAutoTimeOut();     
/********************************************CR1352_0601_Rel_Delete_a_relationship_with_the_CTRL_D_shortcut***********************************************************************/       
   //Choisir aléatoirement une relation à supprimer
//        var randomIndex = Math.round(Math.random()*(arrayOfRelationships.length - 4))
        randomRelationship = arrayOfRelationships[3];
        
        //Sélectionner la relation à supprimer puis faire CTRL + D
        Log.Message("Select a random relationship : '" + randomRelationship + "' and click on the Delete button.");
        Get_ModulesBar_BtnRelationships().Click();
        Get_ModulesBar_BtnRelationships().WaitProperty("IsChecked", true, 100000);
        //Get_Toolbar_BtnRedisplayAllAndRemoveCheckmarks().Click();
        //Delay(2000);
        Get_RelationshipsClientsAccountsGrid().FindChild("Value", randomRelationship, 10).Click();
        Get_RelationshipsClientsAccountsGrid().Keys("^d"); //CTRL + D
