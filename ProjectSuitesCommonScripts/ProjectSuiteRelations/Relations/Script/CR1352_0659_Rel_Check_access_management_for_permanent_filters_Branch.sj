...
/**
    Description : Vérifier la gestion d'accès des filtres permanents - Succursale
    https://jira.croesus.com/testlink/linkto.php?tprojectPrefix=Croes&item=testcase&id=Croes-659
    Analyste d'assurance qualité : Reda Alfaiz
    Analyste d'automatisation : Christophe Paring
*/

function CR1352_0659_Rel_Check_access_management_for_permanent_filters_Branch()
{
    var expectedIACode = "0AED";
    var filterName = "test0659";
    var userNameDARWIC= "DARWIC";
    ...
    try {
        ...
        //SE CONNECTER AVEC L'UTILISATEUR DARWIC ET VÉRIFIER QUE LE FILTRE CRÉÉ AVEC L'UTILISATEUR COPERN EST DISPONIBLE
        
        Terminate_CroesusProcess();
        Login(vServerRelations, userNameDARWIC, psw, language);
        ...
        var filterFound = SearchFilter(filterName);
        if (!filterFound){
            Log.Error(filterName + " filter not found with the user " + userNameDARWIC + " ; this is not expected.");
        }
        else {
            Log.Checkpoint(filterName + " filter found with the user " + userNameDARWIC + " ; this is expected.");
        }
