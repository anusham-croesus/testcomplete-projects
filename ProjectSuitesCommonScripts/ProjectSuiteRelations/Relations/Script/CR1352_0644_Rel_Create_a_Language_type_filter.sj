...
/**
    Description : Créer un filtre type 'Langue'
    https://jira.croesus.com/testlink/linkto.php?tprojectPrefix=Croes&item=testcase&id=Croes-644
    Analyste d'assurance qualité : Reda Alfaiz
    Analyste d'automatisation : Christophe Paring
*/

function CR1352_0644_Rel_Create_a_Language_type_filter()
{
    var temporaryRelationshipName = "#7 TEST";
    var temporaryrelationshipAlternateName = "Test7";
    var filterName = "test09";
    
    try {
        var expectedLanguage = GetData(filePath_Relations, "CR1352", 98, language);
        ...
        for (var i = 0; i < nbOfRelationships; i++){
            ...
            var currentRelationshipLanguage = Get_RelationshipsClientsAccountsGrid().RecordListControl.Items.Item(i).DataItem.get_LanguageDescription();
            if (currentRelationshipLanguage == expectedLanguage){
        ...
        var nbOfLanguages = Get_WinCRUFilter_GrpCondition_DgvValue().WPFObject("RecordListControl", "", 1).Items.get_Count()
        for (var i = 0; i < nbOfLanguages; i++){
            var currentLanguage = Get_WinCRUFilter_GrpCondition_DgvValue().WPFObject("RecordListControl", "", 1).Items.Item(i).DataItem.get_Value();
            if (currentLanguage == expectedLanguage){
                ...
                Get_WinCRUFilter_GrpCondition_DgvValue().Find("Value",expectedLanguage,10).Click();
            }
            else {
