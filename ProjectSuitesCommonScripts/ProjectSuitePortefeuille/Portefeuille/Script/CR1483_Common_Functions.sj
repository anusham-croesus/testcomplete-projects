...
function CR1483_CheckErrorMsgPrefMaxWorkAs(vServer,Button,TabName, FilePath, IdExcel) {
          try {
                    ...
                    Get_WinQuickSearch_TxtSearch().keys("00300");
          ...
          try {
                    ...
                    }else
                    {
                        ...
                        }else
                        {
                            ...
                            Get_WinUserMultiSelection_BtnApply().Click();
                            WaitUntilObjectDisappears(Get_CroesusApp(),"Uid","UserMultiSelectionWindow_e5eb",20000);
...
function CR1483_Check_BranchSelection_IACodes(vServer,Button,BranchSelection) {
          ...
          try {
                   ...
                   // var BranchSelection = "Toronto";
                    ...
                    var indexLigneToronto = Get_WinUserMultiSelection_TabBranches_DgBranches().FindChild(["ClrClassName","WPFControlText"],["CellValuePresenter",BranchSelection],10).Record.index
                    Get_WinUserMultiSelection_TabBranches_DgBranches().WPFObject("RecordListControl", "", 1).WPFObject("DataRecordPresenter", "", indexLigneToronto+1).WaitProperty("IsSelected", true, 60000);
...
function CR1483_Check_UserSelection_IACodes(vServer,Button,UserSelection,IACode1, IACode2) {
          ...
          try {
                    ...
                    //Cliquer sur le boutton ajouter un client pour vérifier que seulement 2 code CP sont autorisés
                    Get_Toolbar_BtnAdd().Click();
                    Get_ClientsGrid_ContextualMenu_Add_CreateExternalClient().Click();
                    
                    //Vérifier que seulement 2 Codes de CP sont affichés dans le combo "BD88" et "OAED"
                    Check_ComboIACode();
...
function Check_ComboIACode()
{
    ...
    if(client == "CIBC"){
        if (NbrItem == 1)  
              Log.Checkpoint("Le Nombre de codes de CP affichés pour COPERN est 1");
        else 
              Log.Error("Le nombre de codes de CP affichés pour COPERN est différent de 1");
      
        if (Get_SubMenus().FindChild(["ClrClassName", "WPFControlText","WPFControlOrdinalNo"], ["ComboBoxItem", "BD88",1], 10))
              Log.Checkpoint("Le code de CP appartient à COPERN existe dans la liste");
        else  Log.Error("Le code de CP n'existe pas dans la liste")  ;
    }
    else{ 
...
function CR1483_Survol_Users_Selection_TabBranches(vServer,Button,filePath) {
          ...
          try {
                    ...
                    var ExpectedFolder = folderPath_Data+"ExportToExcel\\"+client+"\\ExpectedFolder\\Portefolio\\WinSelection\\"+language+"\\";
                    var ResultFolder = folderPath_Data+"ExportToExcel\\"+client+"\\ResultFolder\\Portefolio\\WinSelection\\"+language+"\\";  
...
function CR1483_Survol_Users_Selection_TabIACodes(vServer, Button, filePath) {
          ...
          try {
                    ...
                    //Colonne Branch Code
                    var BranchC = "BranchNumber";
                    //if (client == "CIBC") BranchC = "BranchNumber";
                    Get_WinUserMultiSelection_TabIACodes_ChBranchCode().Click();
                    Check_columnAlphabeticalSort_CR1483( Get_WinUserMultiSelection_TabIACodes_DgvIACodes(),ReadDataFromExcelByRowIDColumnID(filePath, "CR1483", "CR1483_TabIACodes_BranchCode", language+client),BranchC);
                    Get_WinUserMultiSelection_TabIACodes_ChBranchCode().Click();
                    Check_columnAlphabeticalSort_CR1483( Get_WinUserMultiSelection_TabIACodes_DgvIACodes(),ReadDataFromExcelByRowIDColumnID(filePath, "CR1483", "CR1483_TabIACodes_BranchCode", language+client),BranchC); 
...
function CR1483_Survol_Users_Selection_TabUsers(vServer,Button,filePath) {
          ...
          try {
                    ...
                    //Colonne Branch Code
                    var BranchC = "BranchId";
                    //if (client == "CIBC") BranchC = "BranchId";                    
                    Get_WinUserMultiSelection_TabUsers_ChBranchCode().Click();
                    Check_columnAlphabeticalSort_CR1483( Get_WinUserMultiSelection_TabUsers_DgvUsers(),ReadDataFromExcelByRowIDColumnID(filePath, "CR1483", "CR1483_TabUsers_BranchCode", language+client),BranchC);
                    Get_WinUserMultiSelection_TabUsers_ChBranchCode().Click();
                    Check_columnAlphabeticalSort_CR1483( Get_WinUserMultiSelection_TabUsers_DgvUsers(),ReadDataFromExcelByRowIDColumnID(filePath, "CR1483", "CR1483_TabUsers_BranchCode", language+client),BranchC);
          } 
          catch (e) {
