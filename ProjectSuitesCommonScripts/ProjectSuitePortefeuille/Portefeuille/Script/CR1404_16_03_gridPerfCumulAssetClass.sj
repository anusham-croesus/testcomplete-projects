...
/**
    Description : Valider les valeurs calculées de performance pour le compte 800011-NA en période cumulative, par class d'actifs.
    Auteur : Antoine Gélinas
*/
function CR1404_16_03_gridPerfCumulAssetClass()
{
  var testNumber = "16-03-";
  ...
  try {
    ...
    for(col = 0; col < listCol.length && ColumnMarketValue < 0; col++)
    {
    ...
    var summaryResults = Get_Portfolio_AssetClassesGrid().findChild("ClrClassName", "SummaryRecordPresenter", 15).findAllChildren("ClrClassName", "SummaryResultPresenter", 15).toArray().sort(function(a, b){return a.ScreenLeft - b.ScreenLeft});
