...
/*    Date: 24-12-2020
      ...
      Objectif: Combiner les différentes façons de création d'ordre d'achat
*/
 
 function Survol_Por_CreateBuyOrder_CombinedAccess()
 {
   try {
          ...
          var module = ReadDataFromExcelByRowIDColumnID(filePath_Portefeuille, "Survol", "module", language+client);//"portefeuille";
          var order = ReadDataFromExcelByRowIDColumnID(filePath_Portefeuille, "Survol", "order", language+client);//"buy";
          var calledFrom = ReadDataFromExcelByRowIDColumnID(filePath_Portefeuille, "Survol", "calledFrom", language+client);//"CreateABuyOrder";
          var orderStatus = ReadDataFromExcelByRowIDColumnID(filePath_Portefeuille, "Survol", "orderStatus", language+client);//"Creation";
          ...
          var typeBuy = ReadDataFromExcelByRowIDColumnID(filePath_Portefeuille, "Survol", "typeBuy", language+client);//"buy"
          
          var typeStocks = ReadDataFromExcelByRowIDColumnID(filePath_Portefeuille, "Survol", "typeStocks", language+client);//"stocks";
          var positionStocks = ReadDataFromExcelByRowIDColumnID(filePath_Portefeuille, "Survol", "positionStocks", language+client);//"CUF.DB.D"
 ...
 //----------- Étape 2 : Accès à la fenêtre «Orders Module » par Ctrl+Shift+B ---------------------------------------
          ...
          Get_Portfolio_PositionsGrid().Keys("^B");
 ...
 //----------- Étape 4 : Accès à la fenêtre «Orders Module » en cliquant sur le bouton Toolbar-btnCreateBuyOrder ---------------------------------------
          ...
          Check_Properties_FinancialInstrumentSelector(language,typeBuy);// la fonction est dans le CommonCheckpoints
