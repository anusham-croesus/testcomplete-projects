...
Par la suite, le mailler vers le module portefeuille (menuBar-Modules-Portefeuille-Chaîner vers).Rechercher la position R13369. Afficher la fenêtre «Orders Module » par Ctrl+Shift+S
Vérifier le texte et la présence des contrôles */
 
 function Survol_Por_CreateSellOrder_CtrlShiftS_FixedIncome()
 {
   ...
   // Les variables utilisées dans les points de vérifications 
    ...
    var orderStatus = "Creation";
      ...
      if (Get_Toolbar_BtnSearch().WaitProperty("IsVisible", true, 15000)){ Search_Position("R13369")}
      else {Log.Error("The BtnSearch didn't become enabled within 15 seconds.");}  
