...
/**
    ...
    Rechercher un ordre de vente de fonds d'investissement et l'afficher avec le bouton "Consulter" de la barre des Ordres.
    Vérifier le texte et la présence des contrôles.
    @author : christophe.paring@croesus.com
*/

function Survol_Ord_OrdersBar_BtnView_SellMutualFunds()
{
    ...
    if(client == "RJ" || client == "BNC" ||client == "CIBC" || client == "TD" )
    {
        ...
        Check_Properties_CreateOrder_DifType_BE(language, type, module, order, calledFrom, orderStatus);// la fonction n'est plus dans CommonCheckpoints
...
function Check_Properties_CreateOrder_DifType_BE(language, type, module, order, calledFrom, orderStatus)
{
    ...
    if (type == "mutualFunds"){
      ...
      //  if ((calledFrom == "CFO") || (calledFrom == "View")){
            //aqObject.CheckProperty(Get_WinMutualFundsOrderDetail_RdoReinvested(), "IsEnabled", cmpEqual, false); // Fonctionnalité Enlevé pour Be //CROES-8053
       // } else {
     ...
     //   if ((calledFrom == "CFO") || (calledFrom == "View")){
            //aqObject.CheckProperty(Get_WinMutualFundsOrderDetail_RdoCash(), "IsEnabled", cmpEqual, false); // Fonctionnalité Enlevé pour Be //CROES-8053
      //  } else {
       ...
       // }
        ...
        if(Get_WinMutualFundsOrderDetail_RdoReinvested().Exists){
           if (Get_WinMutualFundsOrderDetail_RdoReinvested().VisibleOnScreen)
              Log.Error("Le Bouton Radio Réinvesti existe!")
           else
              Log.Checkpoint("Le Bouton Radio Réinvesti n'existe pas.")
            }
                

        if(Get_WinMutualFundsOrderDetail_RdoCash().Exists){
            if (Get_WinMutualFundsOrderDetail_RdoReinvested().VisibleOnScreen)
                Log.Error("Le Bouton Radio Comptant existe!")
            else
    ...
    if ((calledFrom == "CFO") || (calledFrom == "View")){
        ...
        // Amener la barre de défilement du Grid au bout à droite.
        Get_WinOrderDetail_TabUnderlyingAccounts_DataGrid().Drag(90,272,75,0);
