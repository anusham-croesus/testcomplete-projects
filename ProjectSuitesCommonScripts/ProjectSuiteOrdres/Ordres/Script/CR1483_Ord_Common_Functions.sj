...
function CheckSelectedIACodes(IACode){
    ...
    Log.Message("Le nombre d'ordres qui ont le code de CP "+IACode+" est : "+NbrItem);
    if (NbrItem == 0)
    {
       Log.Checkpoint("La grille ordres est vide pour cette sélection") ;
    }else 
    {
       for (var i=0; i<NbrItem; i++)
       {
            var Item = Get_OrderGrid().RecordListControl.Items.Item(i).DataItem.RepresentativeNumber;
...
function CR1483_Check_BranchSelection_IACodes(vServer,Button,BranchSelection) {
          ...
          try {
                   ...
                   // var BranchSelection = "Toronto";
                    ...
                    //Valider dans la grille ordres que seulement les code de CP qui commence par AC sont affichés
...
function CR1483_Check_UserSelection_IACodes(vServer,Button,UserSelection,IACode1, IACode2) {
          ...
          try {
                    ...
                    //Valider dans la grille Ordres que seulement les code de CP "BD88" et "OAED" sont affichés
...
function CheckSelectedMultiIACodes(IACode1,IACode2){
    ...
    var NbrItem = Get_OrderGrid().RecordListControl.Items.Count;
    Log.Message("Le nombre d'ordres qui ont le code de CP "+IACode1+" et "+ IACode2+" est : "+NbrItem);
    if (NbrItem == 0)
    {
       Log.Checkpoint("La grille Ordres pour cette sélection est vide") ;
    }else {
        for (var i=0; i<NbrItem; i++)
        {
            var Item = Get_OrderGrid().RecordListControl.Items.Item(i).DataItem.RepresentativeNumber;
