...
Version de scriptage: 90.24.2021.04-33 */ 
 ...
 function GDO_2453_Create_BuyOrder_Stocks()
 {             
    try{  
        ...
        var Message11 =ReadDataFromExcelByRowIDColumnID(filePath_Orders, "GDO", "Message1ORC2090A", language+client);
        ...
        CreateEditStocksOrder(account,quantity2,security);
        
        Get_OrderAccumulatorGrid().Find("Value",account,10).DblClick();
        WaitObject(Get_CroesusApp(),"Uid", "OrderDetails_d698");
        aqObject.CheckProperty(Get_WinOrderDetail_TabWarnings(), "IsSelected", cmpEqual,false);
        Get_WinOrderDetail_BtnVerify().Click();
        Log.Message("Valider quand on clic sur vérifier l'onglet Avertissement devient sélectionnée");
        aqObject.CheckProperty(Get_WinOrderDetail_TabWarnings(), "IsSelected", cmpEqual,true);
        ...
        for (i=0; i<count; i++){
           if (i == 0)
              aqObject.CheckProperty(grid.Items.Item(i).DataItem, "Message", cmpEqual,Message1+"\r\n"+Message11);
           else
        ...
        aqObject.CheckProperty(Get_WinAccumulator().Find("Uid", "GroupBox_1c98", 10).Find(["ClrClassName","WPFControlOrdinalNo"],["ListBoxItem","2"],10).DataContext, "Message", cmpEqual,Message2);
