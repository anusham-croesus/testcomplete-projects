...
/* Description : A partir du module « Transactions » , afficher la fenêtre « Info » en cliquant sur EditFunctions_Gainslosses. 
 Vérifier la présence des contrôles et des étiquetés */

function Survol_Tra_MenuBar_EditFunctions_Gainslosses()
{
     ...
     Get_Transactions_ListView().FindChild(["ClrClassName", "WPFControlOrdinalNo"], ["DragableListViewItem", "1"], 10).WaitProperty("VisibleOnScreen", true, 30000);
       ...
       //Search_Transactions_Type(type);
           if(client == "US" || client == "TD" || client == "CIBC"){
              Search_Transactions_Type(type);} 
       //afficher la fenêtre « Info » en cliquant sur EditFunctions_Gainslosses.
       Get_MenuBar_Edit().OpenMenu();
       Get_MenuBar_Edit_FunctionsForTransactions().OpenMenu();
       Get_MenuBar_Edit_FunctionsForTransactions_GainsLosses().Click();
