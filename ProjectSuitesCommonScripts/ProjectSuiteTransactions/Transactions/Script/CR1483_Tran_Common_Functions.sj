...
function CheckSelectedIACodes(IACode){
    ...
    var NbrItem = Get_TransactionsPlugin().WPFObject("listView").Items.Count;
    Log.Message("Le nombre de Transactions qui ont le code de CP "+IACode+" est : "+NbrItem);
    
    var numColonne = 6;         // AJOUTÉ PAR AMINE A.
    if (client == "CIBC")
        numColonne = 4;
    
    if (Get_TransactionsPlugin().WPFObject("listView").Items.Item(i).Item(numColonne))
    for (var i=0; i<NbrItem; i++)
    {
      var Item = Get_TransactionsPlugin().WPFObject("listView").Items.Item(i).Item(numColonne).Data;
      if (Item == IACode) Log.Checkpoint("The item "+i+" is "+IACode);
      else Log.Error("The item "+i+" is different than "+IACode);
    }
}

function Get_Transactions_FirstItem(){
    var width = Get_Transactions_ListView().get_ActualWidth()
    Get_Transactions_ListView().Click(width-10,10)
    WaitObject(Get_CroesusApp(),"Uid", "FixedColumnListView_1b3e",maxWaitTime);
}
...
function Get_GridHeader_ContextualMenu_DefaultConfiguration_CIBC() 
          {return Get_CroesusApp().FindChild(["ClrClassName", "WPFControlText"], ["MenuItem", "Use Default Configuration"], 10)}

function CheckSelectedBranchIACodes(BranchName){
    
     var NbrItem = Get_TransactionsPlugin().WPFObject("listView").Items.Count;
    Log.Message("Le nombre de Transactions de la succursale "+BranchName+" est : "+NbrItem);
...
function CheckSelectedMultiIACodes(IACode1,IACode2){
    ...
    var NbrItem = Get_TransactionsPlugin().WPFObject("listView").Items.Count;
    Log.Message("Le nombre de Transactions qui ont le code de CP "+IACode1+" ,"+IACode2+" est : " + NbrItem);
    
    var debut = 0;          // AJOUTÉ PAR AMINE A.
    var fin = NbrItem;
    var numColonne = 6;
    ...
    if (Get_TransactionsPlugin().WPFObject("listView").Exists){ //Ajouté par Christophe : stabilisation
        var nbOfChecksLeft = 10;
        do {
            var isFound = true;
            var listViewItems = Get_TransactionsPlugin().WPFObject("listView").Items;
            for (var i=debut; i<fin; i++){
                if (listViewItems.Item(i) === null || listViewItems.Item(i).Item(numColonne) === null){
                    nbOfChecksLeft--;
                    isFound = false;
                    Delay(3000);
                    break;
        ...
        } while (!isFound && nbOfChecksLeft > 0)
    }
    
    for (var i=debut; i<fin; i++)
    {
      var Item = Get_TransactionsPlugin().WPFObject("listView").Items.Item(i).Item(numColonne).Data;
      if (Item == IACode1 || Item == IACode2) Log.Checkpoint("The item "+i+" is "+IACode1 +" or "+IACode2 +" : "+Item);
      else Log.Error("The item "+i+" is different than "+IACode1+" and "+IACode2+" : "+Item);
    }
}
