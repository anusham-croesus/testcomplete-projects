...
/* Description : Aller au module "Modeles" :
1- Vérifier le texte des en-têtes.
2- Afficher tous les en-têtes des colonnes dans une table (grid) en appelant le menu contextuel (Clique droit). 
3- Remettre la configuration par défaut.
4- FirmModel - Vérifier la présence des contrôles, des étiquetés et des en-têtes de colonnes dans la partie de détail, en cliquant sur chaque onglet.
5- Basket - Vérifier la présence des contrôles, des étiquetés et des en-têtes de colonnes dans la partie de détail, en cliquant sur chaque onglet.
*/

function Survol_Mod_MainWin_CombinedDetailsHeader()
{
  var type = "firmModel";
  ...
  // 1- Vérifications des en-têtes
  
  Log.Message("Vérifier le texte des en-têtes.");
  //Les points de vérification en français
  if(language=="french"){ Check_HeaderProperties_French();}
  //Les points de vérification en anglais
  else {Check_HeaderProperties_English();}
   
  // 2- Menu contextuel (Clique droit)
  
  Log.Message("Afficher tous les en-têtes des colonnes dans une table (grid) en appelant le menu contextuel (Clique droit).");
  Get_ModelsGrid_ChName().ClickR()
  
  // 3- Remettre configuration par défaut
  
  Log.Message("Remettre la configuration par défaut.");
  Get_GridHeader_ContextualMenu_DefaultConfiguration().Click()
  
  // 4- FirmModel - Vérifer chaque onglet
  
  Log.Message("FirmModel - Vérifier la présence des contrôles, des étiquetés et des en-têtes de colonnes dans la partie de détail, en cliquant sur chaque onglet.");
  ...
  //Les points de vérification FirmModel en français 
   if(language=="french"){ Check_ModelProperties_French(type);}    
  //Les points de vérification FirmModel en anglais 
   else {Check_ModelProperties_English(type);}
   
   Check_Existence_Of_Controls(type);
   
   // 5- Basket - Vérifer chaque onglet
   
   Log.Message("Basket - Vérifier la présence des contrôles, des étiquetés et des en-têtes de colonnes dans la partie de détail, en cliquant sur chaque onglet.");
   if (client == "BNC" ){
       var type = "basket"
       ...
       //Les points de vérification Basket en français 
       if(language=="french"){ Check_ModelProperties_French(type);}
       //Les points de vérification Basket en anglais 
       else {Check_ModelProperties_English(type);}
  }
...
/// Fonctions  (Points de vérification des scripts qui testent Grid_column_header) //////

function Check_HeaderProperties_English()
{
...
//   else
//          aqObject.CheckProperty(Get_ModelsGrid_ChLastUpdate().Content, "OleValue", cmpEqual, "Updated on");
...
function Check_HeaderProperties_French()
{
  Sys.Process("CroesusClient").WPFObject("HwndSource: MainWindow").Maximize() 
   //Default Configuration
   Get_ModelsGrid_ChName().ClickR()
...
/// Fonctions  (Points de vérification des scripts pour la partie Details de fenêtre principal du module Modèle) ///

function Check_ModelProperties_French(type)
{
    Sys.Process("CroesusClient").WPFObject("HwndSource: MainWindow").Maximize() 
    ...
    if (type=="basket")
        aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChQuantity().Content, "OleValue", cmpEqual, "Quantité");
   ...
   //if(type=="basket"){aqObject.CheckProperty(Sys.Process("CroesusClient").WPFObject("HwndSource: PopupRoot", "", 1).WPFObject("PopupRoot", "", 1), "ChildCount", cmpEqual, 16)}; // En réalité dans la liste il y a 15 colonnes qu’on peut ajouter 
    ...
    if (type=="firmModel")
        aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChMVPercent().Content, "OleValue", cmpEqual, "VM (%)"); //Cette ligne a été répétée par ce que la colonne est n’est pas dans le même endroit pour le type basket et firmModel.
    ...
    if (type=="basket")
        aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChQuantity().Content, "OleValue", cmpEqual, "Quantité");
    if (type=="firmModel")
        aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChVariationPercent().Content, "OleValue", cmpEqual, "Écart (%)");
    ...
    if (type=="firmModel")
      aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChSubcategory().Content, "OleValue", cmpEqual, "Sous-catégorie");
    ...
    if (type=="firmModel")
      aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChName().Content, "OleValue", cmpEqual, "Nom");
    ...
    if (type=="firmModel")
        aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChIACode().Content, "OleValue", cmpEqual, "Code de CP"); 
    if (type=="basket")
        aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChMVPercent().Content, "OleValue", cmpEqual, "VM (%)");//Cette ligne a été répétée par ce que la colonne est n’est pas dans le même endroit pour le type basket et firmModel
    ...
    if ( (client == "CIBC" || client == "BNC"  || client == "US" || client == "TD") && type=="firmModel" )
        aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChBeta().Content, "OleValue", cmpEqual, "Bêta");
    ...
    if (type=="basket") { //Remettre l’affichage par default 
    ...
    if (type=="basket")
        aqObject.CheckProperty(Get_Models_Details_TabSummary_ToolBarTray_LblFundAllocation().Text, "OleValue", cmpEqual, "Répartition des fonds");
...
function Check_ModelProperties_English(type)
{
    Sys.Process("CroesusClient").WPFObject("HwndSource: MainWindow").Maximize() 
    ...
    if(type=="basket")
        aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChQuantity().Content, "OleValue", cmpEqual, "Quantity"); 
   ...
   if (type=="firmModel") {
        if (client == "CIBC")
            ...
            aqObject.CheckProperty(Sys.Process("CroesusClient").WPFObject("HwndSource: PopupRoot", "", 1).WPFObject("PopupRoot", "", 1), "ChildCount", cmpEqual, 21);  //EM : 90.09.Er-9 : Avant c'était 19 - Modifié suite à l'activation des PREF RQS 
        else //RJ
            aqObject.CheckProperty(Sys.Process("CroesusClient").WPFObject("HwndSource: PopupRoot", "", 1).WPFObject("PopupRoot", "", 1), "ChildCount", cmpEqual, 18); // En réalité dans la liste il y a 17 colonnes qu’on peut ajouter 
   }
    
   if (type=="basket")
       aqObject.CheckProperty(Sys.Process("CroesusClient").WPFObject("HwndSource: PopupRoot", "", 1).WPFObject("PopupRoot", "", 1), "ChildCount", cmpEqual, 17); // En réalité dans la liste il y a 15 colonnes qu’on peut ajouter 
   ...
   Get_Models_Details_TabPositions_BtnByAssetClassOff_ChDescription().ClickR()
    ...
    if (type=="firmModel"){
        aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChMVPercent().Content, "OleValue", cmpEqual, "MV (%)")};//Cette ligne a été répétée parce que la colonne est n’est pas dans le même endroit pour le type basket et firmModel
    aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChMinMVPercent().Content, "OleValue", cmpEqual, "Min. MV (%)");
    aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChMaxMVPercent().Content, "OleValue", cmpEqual, "Max. MV (%)");
    if(type=="basket"){
        aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChQuantity().Content, "OleValue", cmpEqual, "Quantity")};
    if(type == "firmModel"){
        aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChVariationPercent().Content, "OleValue", cmpEqual, "Variation (%)")};
        aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChDescription().Content, "OleValue", cmpEqual, "Description");
    if(type == "firmModel"){
        aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChSubcategory().Content, "OleValue", cmpEqual, "Subcategory")};
    aqObject.CheckProperty(Get_Models_Details_TabPositions_BtnByAssetClassOff_ChSecurityCurrency().Content, "OleValue", cmpEqual, "Price Currency"); // avant Security Currency
