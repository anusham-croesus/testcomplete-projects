...
//USEUNIT DBA
 ...
 /* Description : Fichier Excel 'Cas de test du CR1452 à automatiser'. 1.Avec un modele séléctionné 
https://jira.croesus.com/testlink/linkto.php?tprojectPrefix=Croes&item=testcase&id=Croes-2219
 
Analyste d'automatisation: Youlia Raisper */


function CR1709_2219_Edit_Price_Model_Firm_SelectedModel()
{
    try{ 
      
        //Afficher le lien de cas de test
        Log.Link("https://jira.croesus.com/testlink/linkto.php?tprojectPrefix=Croes&item=testcase&id=Croes-2219","Cas de test TestLink : Croes-2219")
        ...
        cellPrice.Click();
        cellPrice.WPFObject("ContentControl", "", 1).WPFObject("XamNumericEditor", "", 1).Keys(newPriceABX);
        ...
        // EM: 90-06-Be-17 datapool modifié nouveau prix =638 824.33 - avant "638 824,34" (Apres la reponse de karima Mouzaoui)
        // EM: 90.10.Fm-2 datapool modifié nouveau prix =638 977.63 - avant "638 824,33" (Apres Rép Christine P. : Nouveau comportement valide lié à la correction du JIRA : DEVPRJ-2643)
        ...
        for(j=0;j<filesArray.length;j++){
          if(aqObject.CompareProperty(filesArray[j], cmpContains, fileName ,false,0)==true){
              if(SearchValuesInCSVFile(filesArray[j], [columnSymbol,columnPriceSecurityCurrency,columnMarketValueSecurityCurrency], [securityABX,newPriceABX,marketValueABX])){
                 Log.Checkpoint("Les colonnes et les valeurs suivantes sont présentes dans le fichier CSV : "+ filesArray[j]+ " " +columnSymbol+ "=" + securityABX +" "+ columnPriceSecurityCurrency + "="+ newPriceABX + " " +columnMarketValueSecurityCurrency + "=" +marketValueABX)
              }else{
                Log.Error("Les colonnes et les valeurs suivantes ne sont pas présentes dans le fichier CSV :"+ filesArray[j]+ " " +columnSymbol+ "=" + securityABX +" "+ columnPriceSecurityCurrency + "="+ newPriceABX + " " +columnMarketValueSecurityCurrency + "=" +marketValueABX)
              } 
          } 
          else{
            if(SearchValuesInCSVFile(filesArray[j], [columnSymbol,columnPriceSecurityCurrency], [securityABX,newPriceABX])){
              Log.Checkpoint("Les colonnes et les valeurs suivantes sont présentes dans le fichier CSV : "+ filesArray[j]+ " " +columnSymbol+ "=" + securityABX +" "+ columnPriceSecurityCurrency + "="+ newPriceABX)
            } else{
              Log.Error("Les colonnes et les valeurs suivantes ne sont pas présentes dans le fichier CSV : "+ filesArray[j]+ " " +columnSymbol+ "=" + securityABX +" "+ columnPriceSecurityCurrency + "="+ newPriceABX)
            }
          } 
        }
