...
https://jira.croesus.com/testlink/linkto.php?tprojectPrefix=Croes&item=testcase&id=Croes-3026
 
Analyste d'assurance qualité: Manel
Analyste d'automatisation: Youlia Raisper
La version de scriptage: ref90-04-BNC-59B-5--V9-1_8-co6x */ 
 
 function CR1709_3026_Check_Activation_Intraday_for_SpecificBranch()
 {             
    try{  
        ...
        var account=ReadDataFromExcelByRowIDColumnID(filePath_Modeles, "CR1709", "Account800300NA", language+client);
        var quantity=ReadDataFromExcelByRowIDColumnID(filePath_Modeles, "CR1709", "QuantityStocks_3026", language+client);
        var security= ReadDataFromExcelByRowIDColumnID(filePath_Modeles, "CR1709", "SecurityNA", language+client);
        ...
        aqObject.CheckProperty(Get_PortfolioBar_BtnIntraday(), "VisibleOnScreen", cmpEqual,false);
        
     
        //dans compte --> séléctionner 800300-na --> Créer un ordre achat -->Actions  --> Qte=50 , titre =NA 
        Get_ModulesBar_BtnAccounts().Click();
        Search_Account(account);        
...
function CreateEditStocksOrder(account,quantity,security)
 {    
    var securitysearch = ReadDataFromExcelByRowIDColumnID(filePath_Modeles, "CR1709", "SecurityNA", language+client);
