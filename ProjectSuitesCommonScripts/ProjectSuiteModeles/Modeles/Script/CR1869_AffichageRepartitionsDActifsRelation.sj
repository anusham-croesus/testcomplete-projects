...
function CR1869_AffichageRepartitionsDActifsRelation()
{
  ...
  try
  {
    ...
    var listePortefeuille = GetAllocationsList(Get_PortfolioGrid_GrpSummary_CmbAssetAllocation, Get_PortfolioGrid_GrpSummary_CmbAssetAllocation_Firm, Get_PortfolioGrid_GrpSummary);
    ...
    var listeReequilib = GetAllocationsList(Get_WinRebalance_TabProjectedPortfolios_TabProjectedPortfolio_GrpSummary_CmbAssetAllocation,
                                            Get_WinRebalance_TabProjectedPortfolios_TabProjectedPortfolio_GrpSummary_CmbAssetAllocation_Firm,
...
function GetAllocationsList(comboList, comboFirm, grpSummary)
{
  ...
  var listNames = [];
  for(n = 0; n < names.length; n++)
    if(!aqString.StrMatches("^(\\#(\\w|\\d)(\\w|\\d)(\\w|\\d)(\\w|\\d)(\\w|\\d)(\\w|\\d)(\\w|\\d)(\\w|\\d))|(\\(?\\d+\\.?\\d*\\)?\\%)$", names[n]) &&
                              names[n].length > 0)
