...
function CR1869_CalculEcartCible_Relation()
{
  ...
  var nameRelation = "TEST9329";
  var numberCompte1 = "800049-NA";
  var numberCompte2 = "800062-NA";
  ...
  try
  {
    ...
    var dejaAssocie = Get_Models_Details_TabAssignedPortfolios_DgvAssignedPortfolios().FindChild("Value", nameRelation, 10).Exists;
    if(!dejaAssocie)
    {
      Get_ModulesBar_BtnRelationships().Click();
      Get_Toolbar_BtnAdd().Click();
      Get_Toolbar_BtnAdd_AddDropDownMenu_AddNewRelationship().Click();
      Get_WinDetailedInfo_TabInfo_GrpGeneral_TxtFullName().set_Text(nameRelation);
      Get_WinDetailedInfo_TabInfo_GrpGeneral_TxtIACodeForClient().set_Text("BD88");
      Get_WinDetailedInfo_BtnOK().Click();
      SearchRelationshipByName(nameRelation);
      ...
      Get_WinPickerWindow_DgvElements().Keys(numberCompte1.charAt(0));
      Get_WinQuickSearch_TxtSearch().keys(numberCompte1.slice(1));
      Get_WinQuickSearch_BtnOK().Click();
      Get_WinPickerWindow_DgvElements().FindChild("Value", numberCompte1, 10).Click();
      Get_WinPickerWindow_DgvElements().Keys(numberCompte2.charAt(0));
      Get_WinQuickSearch_TxtSearch().keys(numberCompte2.slice(1));
      Get_WinQuickSearch_BtnOK().Click();
      Get_WinPickerWindow_DgvElements().FindChild("Value", numberCompte2, 10).Click(-1, -1, skCtrl);
  ...
  if(relationAssocie)
  {
    DeleteRelationship(nameRelation);
  }
