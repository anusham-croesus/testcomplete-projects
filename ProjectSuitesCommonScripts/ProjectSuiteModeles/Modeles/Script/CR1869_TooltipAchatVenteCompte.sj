...
function CR1869_TooltipAchatVenteCompte()
{
  ...
  var namePosition, textAccount, textClient, textRelationship;
  if(language == "french")
  {
    namePosition = "ALT COM SURET  /SF/N";
    textAccount = "% du compte";
    textClient = "% du client";
    textRelationship = "% de la relation";
  }
  else
  {
    ...
    textRelationship = "% of the relationship";
  }
  ...
  try
  {
    ...
    aqObject.CheckProperty(menu , "WPFControlText", cmpNotContains, textClient);
    aqObject.CheckProperty(menu , "WPFControlText", cmpNotContains, textRelationship);
  }
  catch(exc)
  {
