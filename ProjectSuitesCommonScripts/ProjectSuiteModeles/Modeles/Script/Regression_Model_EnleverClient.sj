...
function Regression_Model_EnleverClient()
{
  ...
  if(clientAssocie.Exists)
  {
    ...
    clientAssocie = Get_Models_Details_TabAssignedPortfolios_DgvAssignedPortfolios().FindChild("Value", nameClient, 10);
    if(!clientAssocie.Exists)
      Log.Checkpoint("Client enlevé correctment.");
    else
      Log.Error("Client non enlevé.");
  }
  else
