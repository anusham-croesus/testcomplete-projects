...
function ExecutionLog()
{
    ...
    var referenceName = GetVServerReference(vServerModeles);
    var executionEndDateTimeString = aqConvert.DateTimeToFormatStr(aqDateTime.Now(), "%Y_%m_%d %H_%M_%S");
    var projectName = (aqFileSystem.GetFileNameWithoutExtension(ProjectSuite.FileName) == "ProjectSuiteModeles2")? "Modeles2": "Modeles";
    var projectName = GetExecutionLogProjectName(projectName);
    ...
    if (SendMail("christophe.paring@croesus.com;youlia.raisper@croesus.com;sana.ayaz@croesus.com;xian.wei@croesus.com;emna.ibn.hadj.mohamed@croesus.com", "mail.croesus.com", "TestComplete Execution", "testauto@croesus.com", client + " Modeles - exécution des scripts terminée", "L'exécution des scripts du projet Modeles est maintenant terminée."))
