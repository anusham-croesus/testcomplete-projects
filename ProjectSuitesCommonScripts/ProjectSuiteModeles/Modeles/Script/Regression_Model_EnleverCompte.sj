...
function Regression_Model_EnleverCompte()
{
  ...
  var accountAssocie = Get_Models_Details_TabAssignedPortfolios_DgvAssignedPortfolios().FindChild("Value", nameAccount, 10);
  if(accountAssocie.Exists)
  {
    accountAssocie.Click();
    ...
    accountAssocie = Get_Models_Details_TabAssignedPortfolios_DgvAssignedPortfolios().FindChild("Value", nameAccount, 10);
    if(!accountAssocie.Exists)
      Log.Checkpoint("Compte enlevé correctment.");
    else
      Log.Error("Compte non enlevé.");
  }
  else
