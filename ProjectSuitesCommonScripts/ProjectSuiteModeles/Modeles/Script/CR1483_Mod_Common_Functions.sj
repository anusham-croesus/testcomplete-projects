...
function CR1483_RememberMySelection(vServer,Button,TabName, filePath, ItemSelection, ItemSelection2) {
          ...
          try {
                    ...
                    }else
                    {
                        ...
                        }else
                        {
                            ...
                            Log.Message("Validation des codes CP pour la grille Modeles");
                            CheckSelectedIACodes(ItemSelection, ItemSelection2); 
                    ...
                    Get_MenuBar_Users().Click();
                    Get_MenuBar_Users().WaitProperty("IsChecked", true, 30000);
...
//                    aqObject.CheckProperty(Get_MainWindow(), "WndCaption", cmpEqual, ReadDataFromExcelByRowIDColumnID(filePath, "CR1483", "CR1483_WinSelection_Global", language+client));      
                    aqObject.CheckProperty(Get_MainWindow(), "WndCaption", cmpEqual,  "Croesus - Croesus Sysadm (UNI00) -> Global");      
...
function CheckSelectedIACodes(IACode1, IACode2){
    
    var NbrItem = Get_MainWindow().MainWindow.contentContainer.tabControl.ModelsPlugin.modelListView.RecordListControl.Items.Count;
    Log.Message("Le nombre de items qui ont le code de CP "+IACode1+" et "+IACode2+" est : "+NbrItem);
    for (var i=0; i<NbrItem; i++)
    {
      var Item = Get_MainWindow().MainWindow.contentContainer.tabControl.ModelsPlugin.modelListView.RecordListControl.Items.Item(i).DataItem.RepresentativeNumber;
      if (Item == IACode1 || Item == IACode2) Log.Checkpoint("The item "+i+" is correct:"+Item);
      else Log.Error("The item "+i+" is not correct "+Item);
    }
}
...
function CR1483_Check_BranchSelection_IACodes(vServer,Button,BranchSelection, Item1, Item2) {
          ...
          try {
                   ...
                   // var BranchSelection = "Toronto";
                    ...
                    Log.Message("Validation des codes CP pour la grille Modeles");
                    CheckSelectedBranchIACodes(BranchSelection, Item1, Item2); 
...
function CheckSelectedBranchIACodes(BranchName,Item1,Item2){
    var NbrItem = Get_MainWindow().MainWindow.contentContainer.tabControl.ModelsPlugin.modelListView.RecordListControl.Items.Count;
    Log.Message("Le nombre de Items pour la succursale "+BranchName+" est : "+NbrItem);
    for (var i=0; i<NbrItem; i++)
    {
      var Item = Get_MainWindow().MainWindow.contentContainer.tabControl.ModelsPlugin.modelListView.RecordListControl.Items.Item(i).DataItem.RepresentativeNumber;
      var Result1 = aqString.Find(Item, Item1);
      var Position1 = aqConvert.IntToStr(Result1);
      var Result2 = aqString.Find(Item, Item2);
      var Position2 = aqConvert.IntToStr(Result2);
      if ((Result1 != -1 && Position1 == 0)|| (Result2 != -1 && Position2 == 0))
         Log.Checkpoint("The Item "+i +" of IACodes is correct: "+Item);
      else 
         Log.Error("The item "+i+" of IACodes isnot correct: "+Item);
    }
}

function CR1483_Check_UserSelection_IACodes(vServer,Button,UserSelection,IACode1, IACode2, IACode3) {
          ...
          try {
                    ...
                    Get_WinUserMultiSelection_BtnApply().Click();
                    WaitUntilObjectDisappears(Get_CroesusApp(),"Uid","UserMultiSelectionWindow_e5eb",15000);
                    
                    //Valider dans les grilles Dashboard que seulement les code de CP "BD88" et "OAED" sont affichés
                    Log.Message("Validation des codes CP pour la grille Modeles");
                    CheckSelectedMultiIACodes(IACode1,IACode2, IACode3); 
...
function CheckSelectedMultiIACodes(IACode1,IACode2, IACode3){
    ...
    {
      var Item = Get_MainWindow().MainWindow.contentContainer.tabControl.ModelsPlugin.modelListView.RecordListControl.Items.Item(i).DataItem.RepresentativeNumber;
      if (Item == IACode1 || Item == IACode2 || Item == IACode3) 
          Log.Checkpoint("The item "+i+" is correct: "+Item);
      else 
          Log.Error("The item "+i+" is not correct: "+Item);
    }
}
...
function CR1483_Survol_Users_Selection_TabBranches(vServer,Button,filePath) {
          ...
          try {
                    ...
                    var ExpectedFolder = folderPath_Data+"ExportToExcel\\"+client+"\\ExpectedFolder\\Models\\WinSelection\\"+language+"\\";
                    var ResultFolder = folderPath_Data+"ExportToExcel\\"+client+"\\ResultFolder\\Models\\WinSelection\\"+language+"\\";  
