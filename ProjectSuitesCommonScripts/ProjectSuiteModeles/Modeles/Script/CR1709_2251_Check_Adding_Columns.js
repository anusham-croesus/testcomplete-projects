...
function CR1709_2251_Check_Adding_Columns()
{
    try{       
        ...
        if(Get_WinRebalance_TabProjectedPortfolios_TabProposedOrdersBlockOff_ChRealizedGL().Exists && Get_WinRebalance_TabProjectedPortfolios_TabProposedOrdersBlockOff_ChRealizedGL().VisibleOnScreen){
        ...
        }else Log.Message("La colonne RealizedGL n'existe pas.")
        
        if(Get_WinRebalance_TabProjectedPortfolios_TabProposedOrdersBlockOff_ChRealizedGLPercent().Exists && Get_WinRebalance_TabProjectedPortfolios_TabProposedOrdersBlockOff_ChRealizedGLPercent().VisibleOnScreen){
          Get_WinRebalance_TabProjectedPortfolios_TabProposedOrdersBlockOff_ChRealizedGLPercent().ClickR();
          Get_GridHeader_ContextualMenu_RemoveThisColumn().Click();
        }else Log.Message("La colonne RealizedGLPercent n'existe pas.") 
                   
        //*********************************click droit sur la barre --> Ajouter une colonne         
        Get_WinRebalance_TabProjectedPortfolios_TabProposedOrdersBlockOff_ChType().ClickR();
