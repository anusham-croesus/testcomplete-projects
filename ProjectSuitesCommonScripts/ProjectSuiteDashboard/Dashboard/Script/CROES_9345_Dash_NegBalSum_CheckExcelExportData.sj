...
/**
    Module: Dashboard / Negative Cash Balance Summary
    ...
    NB: Ce script couvre le script "CROES_9345_Dash_NegBalSum_ExcelExportDoesNotRespectOrderOfColumnsInGrid", ce dernier n'est pas ajouté au projet d'exécution.
*/


function CROES_9345_Dash_NegBalSum_CheckExcelExportData() {
          ...
          try {
                    var ExpectedFile_DefaultColumns = ReadDataFromExcelByRowIDColumnID(filePath_Dashboard, "Anomalies", "CROES9345_Dash_NegBalSum_DefaultColumnsFile", language+client);
                    var ExpectedFile_AllColumns = ReadDataFromExcelByRowIDColumnID(filePath_Dashboard, "Anomalies", "CROES9345_Dash_NegBalSum_AllColumnsFile", language+client);
                    var ExpectedFile_RandomColumns = ReadDataFromExcelByRowIDColumnID(filePath_Dashboard, "Anomalies", "CROES9345_Dash_NegBalSum_RandomColumnsFile", language+client);
                    var ExpectedFolder = folderPath_Data+"ExportToExcel\\"+client+"\\ExpectedFolder\\Dashboard\\NegativeCashBalanceSummary\\"+language+"\\";
                    var ResultFolder = folderPath_Data+"ExportToExcel\\"+client+"\\ResultFolder\\Dashboard\\NegativeCashBalanceSummary\\"+language+"\\";         
                    ...
                    SetDefaultConfiguration(Get_Dashboard_NegativeCashBalanceSummaryBoard_ChName());
                    
                    //Click the button export excel 
                    Log.Message("Export file to excel");
                    ClickOnExportToExcel(Get_Dashboard_NegativeCashBalanceSummaryBoard()); 
                    ...
                    Add_AllColumnsWithoutProfiles(Get_Dashboard_NegativeCashBalanceSummaryBoard_ChCurrency());
                    Log.Message("Enlever la colonne Âge pour éviter le changement du data pool à chaque anniversaire");
                    DeleteColumn(Get_Dashboard_NegativeCashBalanceSummaryBoard_ChAge());
                    ...
                    DeleteColumn(Get_Dashboard_NegativeCashBalanceSummaryBoard_ChCurrency());
                    MoveColumn(Get_Dashboard_NegativeCashBalanceSummaryBoard_ChName());
