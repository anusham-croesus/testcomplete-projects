...
function CR1452_2435_RebalancingSelectedSleeves()
{
    try{                  
        ...
        var message=ReadDataFromExcelByRowIDColumnID(filePath_Sleeves, "CR1452", "Message_2435", language+client); 
        var quantityXCB=ReadDataFromExcelByRowIDColumnID(filePath_Sleeves, "CR1452", "QuantityXCB_2435", language+client);
        ...
        Get_WinAddPosition_GrpAdd_TxtQuickSearchKey().Keys(positionXCB);
        Get_WinAddPosition_GrpAdd_DlSecurityListPicker().Click()
        Get_WinAddPosition_GrpPositionInformation_TxtQuantity().Keys(quantityXCB);
        ...
        //aqObject.CheckProperty(Get_DlgCroesus_LblMessage(), "Text", cmpEqual, message); // CP : Changé par les deux lignes précédentes pour CO
        //Get_DlgCroesus().Close(); // CP : Changé par les deux lignes précédentes pour CO
