...
function CR1037_C13_Create_Sleeves_Currency()
{
      try{     
      
         var user =ReadDataFromExcelByRowIDColumnID(filePath_ExecutionVServers,"USER", "ROOSEF", "username")            
         var account= ReadDataFromExcelByRowIDColumnID(filePath_Sleeves, "CreationOfSleeves", "AccountNo_C13", language+client);  
         var assetAllocation = ReadDataFromExcelByRowIDColumnID(filePath_Sleeves, "CreationOfSleeves", "WinManagerSleeveCmbAssetAllocationCurrency", language+client);
        ...
        //c)Tous les titres de chaque classe d'actifs sont affichés dans la sleeve correspondante.
         ...
         var assetClassCAD =GetAssetClassItems(Get_Portfolio_AssetClassesGrid_DgvItem(1))// suavgarde les positions de chaque segment de la grille
         Compare_UnderlyingSecurities_vs_AssetClassItems(assetClassCAD,sleeveDescription[0])
         Get_Portfolio_AssetClassesGrid().WPFObject("RecordListControl", "", 1).WPFObject("DataRecordPresenter", "",1).set_IsExpanded(false) // Fermer + 
                 
         var assetClassUSD =GetAssetClassItems(Get_Portfolio_AssetClassesGrid_DgvItem(3))
         Compare_UnderlyingSecurities_vs_AssetClassItems(assetClassUSD,sleeveDescription[2])
