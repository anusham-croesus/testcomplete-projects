...
function Restore_ChangeNonRedeemablePosition_ForRJ()
{
 if(client=="RJ"){
   try{             
        ...
        ChangeNonRedeemable(positionFTS,false)
        Get_ModulesBar_BtnAccounts().Click();
        Search_Account(account)      
        Get_RelationshipsClientsAccountsGrid().Find("Value",account,10).Click();  
        //chainer vers le module Portefeuille,
        ...
        aqObject.CheckProperty(Get_Portfolio_AssetClassesGrid().WPFObject("RecordListControl", "", 1).Find("Value",positionFTS,10).DataContext.DataItem, "IsSecurityLegacy", cmpEqual,false);              
