...
function SetReportParameters(asOfDate, checkIncludeGraph, type, checkComparative, checkGroupByRegion, checkGroupByIndustryCode, groupByIndustryCode, checkGroupByAccountCurrency, costCalculation, checkCostDisplayedTheoreticalValue, assetAllocation, customAllocation, checkUseTheSpecifiedInvestmentObjective, checkFundBreakdownClassBreakdown, checkFundBreakdownAppendix, numbering, sortBy, isTheoreticalValueToBeValidated)
{
    ...
    if (isTheoreticalValueToBeValidated == undefined || isTheoreticalValueToBeValidated !== true){
        ...
        var expectedValueForTheoreticalValue = GetBooleanValue(checkCostDisplayedTheoreticalValue)
        if (expectedValueForTheoreticalValue != Get_WinParameters_GrpCostDisplayed_ChkTheoreticalValue().IsChecked.OleValue)
            Get_WinParameters_GrpCostDisplayed_ChkTheoreticalValue().Click();
        Get_WinParameters_GrpCostDisplayed_ChkTheoreticalValue().WaitProperty("IsChecked.OleValue", expectedValueForTheoreticalValue, 5000);
        CheckEquals(Get_WinParameters_GrpCostDisplayed_ChkTheoreticalValue().IsChecked.OleValue, expectedValueForTheoreticalValue, "L'état du paramètre 'Théorical Value'");
    }
