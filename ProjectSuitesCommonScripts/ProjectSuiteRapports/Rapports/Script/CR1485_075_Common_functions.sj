...
{
    Activate_Inactivate_Pref(userNameReportsCR1485, "PREF_REPORT_PERFORMANCE_HISTO", "YES", vServerReportsCR1485);
...
function SetReportParameters(CheckExcludeDataPrecedingTheManagementStartDate, startDate, endDate, checkDisplayDefaultIndices, indicesToBeChecked, checkUseIndexBaseCurrency, checkTimeWeightedNetOfFees, checkTimeWeightedGrossOfFees, checkMoneyWeightedNetOfFees, checkIncludeGraph, data, performanceCalculations, numbering)
{
    ...
    if (Get_WinParameters_GrpGraph_ChkIncludeGraph().IsEnabled)
        Get_WinParameters_GrpGraph_ChkIncludeGraph().set_IsChecked(aqString.ToUpper(checkIncludeGraph) == "VRAI" || aqString.ToUpper(checkIncludeGraph) == "TRUE");    
