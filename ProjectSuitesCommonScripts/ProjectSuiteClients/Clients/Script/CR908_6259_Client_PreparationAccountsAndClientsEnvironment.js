...
/**
     ...
     https://jira.croesus.com/testlink/linkto.php?tprojectPrefix=Croes&item=testcase&id=Croes-6259
    Auteur : Sana Ayaz
...
function CR908_6259_Client_PreparationAccountsAndClientsEnvironment()
{
    try {
          Log.Link("https://jira.croesus.com/testlink/linkto.php?tprojectPrefix=Croes&item=testcase&id=Croes-6259");
          //Les variables
          //Les numéros des comptes
           ...
           //	1. Se loguer dans le configurateur au niveau FIRME et mettre la préférence PREF_ENABLE_CLIENT_STATUS = 1
           ...
           RestartServices(vServerClients);
            /*2	
              ...
              select no_compte, status from b_compte where no_compte in ('800222-LV','800240-NA', '800240-OB', '800253-LY', '800253-NA', '800078-NA', '800078-OB',   '800015-RE',   '800015-SF' ) */
               ...
               Execute_SQLQuery(" update b_compte set status=2 where no_compte ='"+numberAccomp800222LV+"'",vServerClients)   
               Execute_SQLQuery(" update b_compte set status=2 where no_compte ='"+numberAccomp800240NA+"'",vServerClients)
               Execute_SQLQuery(" update b_compte set status=2 where no_compte ='"+numberAccomp800240OB+"'",vServerClients)
               Execute_SQLQuery(" update b_compte set status=2 where no_compte ='"+numberAccomp800253LY+"'",vServerClients)
               Execute_SQLQuery(" update b_compte set status=3 where no_compte ='"+numberAccomp800253NA+"'",vServerClients)
               Execute_SQLQuery(" update b_compte set status=3 where no_compte ='"+numberAccomp800078NA+"'",vServerClients)
               Execute_SQLQuery(" update b_compte set status=3 where no_compte ='"+numberAccomp800078OB+"'",vServerClients)
               Execute_SQLQuery(" update b_compte set status=2 where no_compte ='"+numberAccomp800015RE+"'",vServerClients)
               Execute_SQLQuery(" update b_compte set status=3 where no_compte ='"+numberAccomp800015SF+"'",vServerClients)
               
               /************************************************Adaptation des scripts******************************************************************************************************************/
              var arrayAccompt = new Array();
              arrayAccompt[0]=Execute_SQLQuery_GetFieldAllValues("select status from b_compte where no_compte = '"+numberAccomp800015RE+"'", vServerClients, "status");
              arrayAccompt[1]=Execute_SQLQuery_GetFieldAllValues("select status from b_compte where no_compte = '"+numberAccomp800015SF+"'", vServerClients, "status");
              arrayAccompt[2]=Execute_SQLQuery_GetFieldAllValues("select status from b_compte where no_compte = '"+numberAccomp800078NA+"'", vServerClients, "status");
              arrayAccompt[3]=Execute_SQLQuery_GetFieldAllValues("select status from b_compte where no_compte = '"+numberAccomp800078OB+"'", vServerClients, "status");
              arrayAccompt[4]=Execute_SQLQuery_GetFieldAllValues("select status from b_compte where no_compte = '"+numberAccomp800222LV+"'", vServerClients, "status");
              arrayAccompt[5]=Execute_SQLQuery_GetFieldAllValues("select status from b_compte where no_compte = '"+numberAccomp800240NA+"'", vServerClients, "status"); 
              arrayAccompt[6]=Execute_SQLQuery_GetFieldAllValues("select status from b_compte where no_compte = '"+numberAccomp800240OB+"'", vServerClients, "status"); 
              arrayAccompt[7]=Execute_SQLQuery_GetFieldAllValues("select status from b_compte where no_compte = '"+numberAccomp800253LY+"'", vServerClients, "status"); 
              arrayAccompt[8]=Execute_SQLQuery_GetFieldAllValues("select status from b_compte where no_compte = '"+numberAccomp800253NA+"'", vServerClients, "status"); 
              ...
              // var arrayStatusAccompt = new Array(2,3,2,3,3,3,2,2,2);//SA: Modifé puisque sur la version :90.12.18 l'ordre affiché des comptes suite a la requête select est différent par rapport a avant
              // if (client == "BNC" )
                  arrayStatusAccompt = new Array(2,3,3,3,2,2,2,2,3);
              ...
              for(i=0; i<9; i++)
               ...
               var arrayStatusClient = new Array(1,3,1,2,2);
