...
*/
function CROES_9031_TitrMissingColumnsInTheNotesGrid()
{
    try {
        ...
        Activate_Inactivate_Pref("ROOSEF","PREF_EDIT_SECURITY_NOTE","YES",vServerTitre)
        RestartServices(vServerTitre); 
        //Se connecter avec ROOSEF
        Login(vServerTitre, userNameROOSEF, passwordROOSEF, language);
        var NumberSecurityCROES9031=ReadDataFromExcelByRowIDColumnID(filePath_Titre, "Anomalies", "NumberSecurityCROES9031", language+client);
        var TextNotCROES_9031=ReadDataFromExcelByRowIDColumnID(filePath_Titre, "Anomalies", "TextNotCROES_9031", language+client);
        var TextNotCROES_9031Modif=ReadDataFromExcelByRowIDColumnID(filePath_Titre, "Anomalies", "TextNotCROES_9031Modif", language+client);
        Get_ModulesBar_BtnSecurities().Click();
        //Ajouter une note ou modifier une note 
        Search_Security(NumberSecurityCROES9031)
        
        Get_SecurityGrid().FindChild("Value", NumberSecurityCROES9031, 10).Click();
        Get_SecurityGrid().FindChild("Value", NumberSecurityCROES9031, 10).DblClick();
       ...
       Get_WinInfoSecurity_BtnOK().Click();
        ...
        Get_SecurityGrid().FindChild("Value", NumberSecurityCROES9031, 10).Click();
        Get_SecurityGrid().FindChild("Value", NumberSecurityCROES9031, 10).DblClick();
        Get_WinInfoSecurity().WaitProperty("VisibleOnScreen", true, 500)
        Get_WinInfoSecurity_Notes().Click();
        Get_WinInfo_Notes_TabGrid().Click();
        ...
        Activate_Inactivate_Pref("ROOSEF","PREF_EDIT_SECURITY_NOTE","NO",vServerTitre)
        RestartServices(vServerTitre); 
