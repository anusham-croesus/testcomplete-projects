...
function PreExecution()
{
    ...
    if (aqFileSystem.GetFileNameWithoutExtension(ProjectSuite.FileName) != "ProjectSuiteMiniRegression" || GetIndexOfItemInArray(["PreExecution", (GetProjectSuiteEnabledProjectsNamesList()[0])], aqFileSystem.GetFileNameWithoutExtension(Project.FileName)) != -1){
        DisableDoubleAuthentication(vServerBilling, false);//Désactiver la double authentification
    ...
    RestartServicesAndCheckVServer(vServerBilling); //Par défaut, toute l'exécution s'arrête en cas d'échec de la vérification du VServer
    //UpdateClientVserverAssembleScript(vServerBilling, true); //Désactivée car la MAJ de l'Assemble Script est faite par Jenkins
}