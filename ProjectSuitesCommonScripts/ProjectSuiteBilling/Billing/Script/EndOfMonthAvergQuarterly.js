...
function EndOfMonthAvergQuarterly()
{
      ...
      try {
              ...
              var RelationShipName=GetData(filePath_Billing,"RelationBilling",100,language)
          ...
          // double click sur la relation Billing Relation
            ...
            Delay(3000);
            Get_WinDetailedInfo_BtnApply().Click();
            FillInTheBottomPartOfAccounts(63);
             ...
             Log.Message("Le nombre des éléments de la grille de la grille des messages est"+count);
              ...
              if(count==2){
                    for(i=0;i<count;i++)
                    {
                           ...
                           var messagesExcel=GetData(filePath_Billing,NameSheetEnd_month_averag_Q,162+i,language)
                           Log.Message(162+i)
              ...
              if(count==2){
                    for(i=0;i<count;i++)
                    {
                           ...
                           var messagesExcel=GetData(filePath_Billing,NameSheetEnd_month_averag_Q,224+i,language)
                           Log.Message(224+i)
...
for(var j=0;j<20;j++){ 
    // Vérifier la partie des comptes
             ...
             aqObject.CheckProperty(Get_WinBilling_GrpSummaryCAD_TxtBillingDate(), "WPFControlText", cmpEqual, GetData(filePath_Billing,NameSheetEnd_month_averag_Q,216,language));
             
             aqObject.CheckProperty(Get_WinBilling_GrpSummaryCAD_TxtTotalFees(), "Enabled", cmpEqual, true);
             aqObject.CheckProperty(Get_WinBilling_GrpSummaryCAD_TxtTotalFees(), "WPFControlText", cmpEqual, GetData(filePath_Billing,NameSheetEnd_month_averag_Q,217,language));
            
             aqObject.CheckProperty( Get_WinBilling_GrpSummaryCAD_TxtIATotalFeesBD88(), "WPFControlText", cmpEqual, GetData(filePath_Billing,NameSheetEnd_month_averag_Q,218,language));
             aqObject.CheckProperty( Get_WinBilling_GrpSummaryCAD_TxtIATotalFees9047763(), "WPFControlText", cmpEqual, GetData(filePath_Billing,NameSheetEnd_month_averag_Q,219,language));
            ...
            Get_WinDetailedInfo_TabBillingForRelationship().Click();
            Delay(800)
            FillTheCashFlowAdjustmen (GetData(filePath_Billing,"RelationBilling",165,language),GetData(filePath_Billing,"RelationBilling",165,language),GetData(filePath_Billing,"RelationBilling",167,language),GetData(filePath_Billing,"RelationBilling",167,language));
