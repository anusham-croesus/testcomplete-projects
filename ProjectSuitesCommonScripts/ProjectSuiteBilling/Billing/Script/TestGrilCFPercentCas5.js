...
-Cash Flow Adjustement: percentage/Inflow : 3, percentage/Outflow:2, Amount/Inflow:2,400.00 et Amount/OutFlow=1,170.00.
...
Analyste d'automatisation: Sana Ayaz */
 ...
 function TestGrilCFPercentCas5()
  {
      try {
      
            var RelationshipName=GetData(filePath_Billing,"RelationBilling",36,language)
            
			      var TxtPercentageInflow=GetData(filePath_Billing,"RelationBilling",50,language)
            var TxtPercentageOutflow=GetData(filePath_Billing,"RelationBilling",46,language)
            var TxtAmountInflow=GetData(filePath_Billing,"RelationBilling",51,language)
            var TxtAmountOutflow=GetData(filePath_Billing,"RelationBilling",52,language)
            ...
            // Cliquer sur le bouton OK
             ...
             if(count==2)
             {
                    for(i=0;i<count;i++)
                    {
                           ...
                           var messagesExcel=GetData(filePath_Billing,"RelationBilling",59+i,language)
