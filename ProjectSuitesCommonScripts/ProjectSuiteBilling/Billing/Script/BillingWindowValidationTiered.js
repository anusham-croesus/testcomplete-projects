...
function GenerateAndValidateBillingWindowTiered(executedScript)
{
  try
  {
    ...
    var numberFeesTiers = excelSheet.Cell(excelSheetColumn, excelSheetRelationshipsStartRow + numberColumnsRelationshipsGroupbox + 1).Value;
    var excelSheetFeesTiersStartRow = excelSheetRelationshipsStartRow + numberColumnsRelationshipsGroupbox + 2;
    ...
    for (var currentElementRelationshipsGroupbox = 0; currentElementRelationshipsGroupbox < expectedNumberElementsRelationshipsGroupbox; currentElementRelationshipsGroupbox++)
    {
      ...
      for (var currentRelationshipColumn = 0; currentRelationshipColumn < numberColumnsRelationshipsGroupbox; currentRelationshipColumn++)
      {
        currentRelationship.push(excelSheet.Cell(excelSheetColumn, (excelSheetRelationshipsStartRow + currentRelationshipColumn)).Value);
      }
      
      for (var currentFeesTiersRow = 0; currentFeesTiersRow < numberFeesTiers; currentFeesTiersRow++)
      {
        var currentFeesTiers = [];
              
        for (var currentFeesTiersColumn = 0; currentFeesTiersColumn < numberColumnsFeesExpandableFieldRecord; currentFeesTiersColumn++)
        {
          currentFeesTiers.push(excelSheet.Cell(excelSheetColumn, excelSheetFeesTiersStartRow + currentFeesTiersColumn));
        }
        
        currentRelationship.push(currentFeesTiers);
        excelSheetFeesTiersStartRow += numberColumnsFeesExpandableFieldRecord;     
      }
            
      expectedRelationshipsGroupbox.push(currentRelationship);     
      excelSheetRelationshipsStartRow = excelSheetRelationshipsStartRow + numberColumnsRelationshipsGroupbox + (numberFeesTiers * numberColumnsFeesExpandableFieldRecord) + 3;
      excelSheetFeesTiersStartRow = excelSheetRelationshipsStartRow + numberColumnsRelationshipsGroupbox + 2;
      numberFeesTiers = excelSheet.Cell(excelSheetColumn, excelSheetFeesTiersStartRow - 1).Value;  
    }
    ...
    for (var currentBillingRow = 0; currentBillingRow < expectedNumberElementsRelationshipsGroupbox; currentBillingRow++)
    {
      ...
      for (var currentAccountRow = 0; currentAccountRow < numberElementsAccountsGroupbox; currentAccountRow++)
      {
        ...
        excelSheetAccountsStartRow = excelSheetAccountsStartRow + numberColumnsAccountsGroupbox + 1;
      }
    ...
    // Retrieve Relationships, Accounts, Fees Asset Classes, and Messages groupboxes column names
    //
    ...
    var firstBillingPeriod = aqConvert.VarToStr(expectedRelationshipsGroupbox[0][3]);
    propertiesArray = ["ClrClassName", "WPFControlText"];
    valuesArray = ["CellValuePresenter", firstBillingPeriod];
    var firstRowDataRecordPresenterObject = Get_WinBilling_GrpRelationships_DgvRelationships().WPFObject("RecordListControl", "", 1).Find(propertiesArray, valuesArray, 100).Parent.Parent.Parent;
    
    if (firstRowDataRecordPresenterObject.ExpansionIndicatorVisibility.OleValue == "Visible")
    {
      firstRowDataRecordPresenterObject.set_IsExpanded(true);
      var columnNamesFeesTiersExpandable = GetColumnNames(firstRowDataRecordPresenterObject.WPFObject("RecordListControl", "", 1).WPFObject("ExpandableFieldRecordPresenter", "", 1).WPFObject("RecordListControl", "", 1), numberColumnsFeesExpandableFieldRecord);
      firstRowDataRecordPresenterObject.set_IsExpanded(false);
    }    
    ...
    // Validating each row from the Relationships groupbox. While validating Relationships groupbox row, validate corresponding Fees Tiered values abd Accounts in Accounts Groupbox
    //
    for (var currentRowRelationshipsGroupbox = 0; currentRowRelationshipsGroupbox < actualNumberElementsRelationshipsGroupbox; currentRowRelationshipsGroupbox++)
    {
      ...
      var currentRelationshipsContentItemGrid = Get_WinBilling_GrpRelationships_DgvRelationships().WPFObject("RecordListControl", "", 1).Find("Value", currentBillingPeriod, 100).Parent;
      ...
      currentRelationshipsContentItemGrid.Click();
      currentRelationshipsContentItemGrid.Refresh();
      ...
      for (var currentColumnRelationshipsGroupbox = 0; currentColumnRelationshipsGroupbox < numberColumnsRelationshipsGroupbox; currentColumnRelationshipsGroupbox++)
      {
        ...
        currentActual = aqConvert.VarToStr(currentRelationshipsContentItemGrid.FindChild("WPFControlOrdinalNo", (currentColumnRelationshipsGroupbox + 1), 100).WPFControlText);
      ...
      // Validating Fees expandable record for current Relationship billing period
      //
      Log.PopLogFolder();
      Log.AppendFolder("Validation of Fee Tiers.");
      Log.AppendFolder("Validation of number of Fee Tiers for Billing period " + currentBillingPeriod.substr(0, 10) + ".");      

      var currentRelationshipsDataRecordPresenter = GetNthParent(currentRelationshipsContentItemGrid, 2);
      
      if (currentRelationshipsDataRecordPresenter.ExpansionIndicatorVisibility.OleValue == "Visible")
      {
        currentRelationshipsDataRecordPresenter.set_IsExpanded(true);
        var currentFeeTierRecordListControl = currentRelationshipsDataRecordPresenter.FindChild("ClrClassName", "DataRecordPresenter", 3).Parent;
        
        var actualNumberFeeTiers = currentFeeTierRecordListControl.Items.Count;
        var expectedNumberFeeTiers = expectedRelationshipsGroupbox[currentRowRelationshipsGroupbox].length - numberColumnsRelationshipsGroupbox;
        
        if (actualNumberFeeTiers == expectedNumberFeeTiers)
        {
          Log.Checkpoint("Expecting " + expectedNumberFeeTiers + " Fee Tiers." + " Found " + actualNumberFeeTiers + " Fee Tiers as expected.");
          Log.PopLogFolder();
          
          // Validate all the actual fee tiers
          //
          propertiesArray = ["ClrClassName", "WPFControlOrdinalNo"];
          for (var currentFeeTierRow = 0; currentFeeTierRow < actualNumberFeeTiers; currentFeeTierRow++)    
          {
            valuesArray = ["DataRecordPresenter", currentFeeTierRow + 1];
            var currentFeeTierDataRecordPresenter = currentFeeTierRecordListControl.Find(propertiesArray, valuesArray, 100);
            Log.AppendFolder("Validating Fee Tier " + (currentFeeTierRow + 1) + " of " + actualNumberFeeTiers + ".");
          
            for (var currentFeeTiersColumn = 0; currentFeeTiersColumn < numberColumnsFeesExpandableFieldRecord; currentFeeTiersColumn++)
            {
              valuesArray = ["CellValuePresenter", currentFeeTiersColumn + 1];
              currentExpected = aqConvert.VarToStr(expectedRelationshipsGroupbox[currentRowRelationshipsGroupbox][numberColumnsRelationshipsGroupbox + currentFeeTierRow][currentFeeTiersColumn].Value);
              currentActual = currentFeeTierDataRecordPresenter.FindChild(propertiesArray, valuesArray, 100).WPFControlText;
              currentColumnName = columnNamesFeesTiersExpandable[currentFeeTiersColumn];
              CheckIfEqual(currentActual, currentExpected, "Validation of column '" + currentColumnName + "'."); 
            }
        ...
        {
          Log.Error("Expecting " + expectedNumberFeeTiers + " Fee Tiers." + " Found " + actualNumberFeeTiers + " Fee Tiers instead of the expected " + expectedNumberFeeTiers + ".");
          Log.Message("Validation of asset classes for account '" + currentAccountNumber + "' will not continue.");
          Log.PopLogFolder();
        }

        currentRelationshipsDataRecordPresenter.set_IsExpanded(false);
        Log.PopLogFolder(); 
      }
      ...
      var expectedNumberAccounts = aqConvert.StrToInt(currentRelationshipsContentItemGrid.FindChild("WPFControlOrdinalNo", "17", 100).WPFControlText);
