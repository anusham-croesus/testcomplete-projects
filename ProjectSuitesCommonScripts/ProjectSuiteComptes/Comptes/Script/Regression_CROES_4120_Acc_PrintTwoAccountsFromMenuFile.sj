...
function Regression_CROES_4120_Acc_PrintTwoAccountsFromMenuFile()
{
  try
  {
    ...
    var Ncompte1 = ReadDataFromExcelByRowIDColumnID(filePath_Accounts, "Regression", "Ncompte1", language+client);
    var Ncompte2 = ReadDataFromExcelByRowIDColumnID(filePath_Accounts, "Regression", "Ncompte2", language+client);
    ...
    Log.AppendFolder("Select 2 accounts, print from File menu and validate that .pdf file is correctly saved.");
    Get_ModulesBar_BtnAccounts().Click();
    
    // Créer un tableau contenant les numéros de comptes 
    var comptes = new Array(Ncompte1, Ncompte2);
    
    for (var i = 0 ; i < comptes.length; i++)
    {                  
      Log.Message("Account Number: '" + comptes[i] + "'");
    }
  ...
  {   
    ...
    Log.Message("Restore default column configuration.");
