...
function PreExecution()
{
    ...
    if (aqFileSystem.GetFileNameWithoutExtension(ProjectSuite.FileName) != "ProjectSuiteMiniRegression" || GetIndexOfItemInArray(["PreExecution", (GetProjectSuiteEnabledProjectsNamesList()[0])], aqFileSystem.GetFileNameWithoutExtension(Project.FileName)) != -1){
        DisableDoubleAuthentication(vServerAccounts, false);//Désactiver la double authentification
    ...
    RestartServicesAndCheckVServer(vServerAccounts); //Par défaut, toute l'exécution s'arrête en cas d'échec de la vérification du VServer
    //UpdateClientVserverAssembleScript(vServerAccounts, true); //Désactivée car la MAJ de l'Assemble Script est faite par Jenkins
}