...
/* Description : À partir du module « Comptes », afficher la fenêtre « Gestion de portefeuilles » en cliquant sur MenuBar -File- BtnQLockTheApplication . 
 Vérifier la présence des contrôles et des étiquettes. Par la suite, cliquer sur le btn Ok sans donner un psw, vérifier la présence d’un message */
  
function Survol_Acc_MenuBar_FileLock_BtnOk_WithoutPSW()
{
 ...
 /* var width=Get_DlgCroesus().get_ActualWidth()
  ...
  WaitUntilObjectDisappears(Get_CroesusApp(), ["ClrClassName", "Title"], ["BaseWindow", "Croesus"]);
