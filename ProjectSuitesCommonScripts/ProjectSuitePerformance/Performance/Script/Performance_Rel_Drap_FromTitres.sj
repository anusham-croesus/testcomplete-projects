...
function Performance_Rel_Drap_FromTitres() {

            var StopWatchObj = HISUtils.StopWatch;
            var SoughtForValue = "Performance_Rel_Drap_FromTitres";
...
//        
//                // Maillage un titre au module relations
//                StopWatchObj.Start();
//                Drag(Get_SecurityGrid().FindChild(["ClrClassName", "WPFControlOrdinalNo"], ["DataRecordPresenter", posTitre], 10), Get_ModulesBar_BtnRelationships());
...
//            } else if (DataType == "Data"){
                ...
                // Maillage un titre au module relations
                StopWatchObj.Start();
                Drag(Get_SecurityGrid().Find("Value",titre,1000), Get_ModulesBar_BtnRelationships());
                WaitObject(Get_RelationshipsClientsAccountsPlugin(), "ClrFullClassName", "Infragistics.Windows.DataPresenter.RecordListControl", waitTimeShort);
                Get_RelationshipsClientsAccountsGrid().FindChild(["ClrClassName", "WPFControlOrdinalNo"], ["DataRecordPresenter", 1], 10).WaitProperty("VisibleOnScreen", true, 15000);
                StopWatchObj.Stop();
                //*******************************************************************
