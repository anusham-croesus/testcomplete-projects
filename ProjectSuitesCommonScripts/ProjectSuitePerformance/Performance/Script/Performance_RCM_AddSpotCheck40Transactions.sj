...
function Performance_RCM_AddSpotCheck40Transactions(){

        var StopWatchObj   = HISUtils.StopWatch;
        var SoughtForValue = "Performance_RCM_AddSpotCheck40Transactions";
        var waitTimeShort  = ReadDataFromExcelByRowIDColumnID(filePath_Performance, sheetName_DataBD, "ShortTime", language+client);
        var waitTime = 5000;
        //Nombre de transaction à selectionner
        var nbTansaction = 40;
        //Texte de la note
        var noteText = "RCM ajout de Spot Check, 40 transactions";
        ...
        try {
          ...
          //Selectionner 40 transactions
          for (i=0; i<nbTansaction; i++){
              itemI = Get_WinRQS_TabTransactionBlotter_BlotterControl().WPFObject("_transactionList").WPFObject("RecordListControl", "", 1).Items.Item(i);
              if(itemI.DataItem.ValidationStatus == null){
                  itemI.set_IsSelected(true)}
              else 
                nbTansaction++;
          }
         ...
         StopWatchObj.Stop();
          ...
          //Supprimer les spot Ckeck
          //Fermer et ouvrir la fenêtre RQS
          /*Aliases.CroesusApp.WPFObject("HwndSource: _activityBlotterManagerWindow").Keys("~[F4]");
          WaitUntilObjectDisappears(Get_CroesusApp(), "Uid", "TabItem_a70d", waitTime);
          Get_Toolbar_BtnRQS().Click();
          WaitObject(Get_CroesusApp(), "Uid", "TabItem_a70d", waitTime);
          Get_WinRQS_TabTransactionBlotter().WaitProperty("Enabled", true, waitTime)*/  
