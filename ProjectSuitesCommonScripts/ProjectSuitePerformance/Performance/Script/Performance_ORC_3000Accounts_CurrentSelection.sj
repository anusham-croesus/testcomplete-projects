...
function Performance_ORC_3000Accounts_CurrentSelection()
{
        var cmbTransactionCurrentSelection = ReadDataFromExcelByRowIDColumnID(filePath_Performance, "DataPerformanceNFR", "cmbTransactionUnitsPerAccount", language+client);        
        var SoughtForValueCurrentSelection = ReadDataFromExcelByRowIDColumnID(filePath_Performance, "DataPerformanceNFR", "SoughtForValueCurrentSelection", language+client);        
        
        Performance_ORC_3000Accounts(cmbTransactionCurrentSelection, SoughtForValueCurrentSelection)
...
//************************** Étape 3 *********************************************************************************
        Log.PopLogFolder();
        logEtape3 = Log.AppendFolder("Étape 3: Ajouter transactions équivalentes (Achat) (50% (MSFT) et 50% (AAPL))");
        
        PurchasequivalentTransactAddiction(qtyBuyORC3000, symbolMSFT);  //Répartition: 50% (NA) et 
        PurchasequivalentTransactAddiction(qtyBuyORC3000, symbolAAPL);   //Répartition: 50% (NBC181)
