...
function Performance_Cli_Drag_FromTitres() {

            var StopWatchObj = HISUtils.StopWatch;
            var SoughtForValue = "Performance_Cli_Drag_FromTitres";
...
//    
//                // Maillage un titre au module clients
//                StopWatchObj.Start();
//                Drag(Get_SecurityGrid().FindChild(["ClrClassName", "WPFControlOrdinalNo"], ["DataRecordPresenter", posTitre], 10), Get_ModulesBar_BtnClients());
...
//            } else if (DataType == "Data"){
                ...
                Get_SecurityGrid().FindChild(["ClrClassName", "WPFControlOrdinalNo"], ["DataRecordPresenter", "1"], 10).WaitProperty("IsLoaded", true, waitTimeShort);
    
                // Maillage un titre au module Clients
                StopWatchObj.Start();
                Drag(Get_SecurityGrid().Find("Value",titre,1000), Get_ModulesBar_BtnClients());
                WaitObject(Get_CroesusApp(), ["ClrClassName", "WPFControlOrdinalNo"], ["ToggleButton", 1],waitTimeShort);
                StopWatchObj.Stop();
                //*******************************************************************
//            }
            ...
            Get_ModulesBar_BtnSecurities().Click(); 
            WaitObject(Get_CroesusApp(), "Uid", "DataGrid_865b");
            Get_SecurityGrid().FindChild(["ClrClassName", "WPFControlOrdinalNo"], ["DataRecordPresenter", "1"], 10).WaitProperty("VisibleOnScreen", true, waitTimeShort);
            Get_Toolbar_BtnRedisplayAllAndRemoveCheckmarks().Click();
