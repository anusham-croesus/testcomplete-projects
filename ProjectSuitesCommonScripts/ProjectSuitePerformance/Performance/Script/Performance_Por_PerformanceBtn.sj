...
function Performance_Por_PerformanceBtn(){

        var StopWatchObj = HISUtils.StopWatch;
        var SoughtForValue = "Performance_Por_PerformanceBtn";
...
//        var waitTimeShort = GetData(filePath_Performance, sheetName_DataBD, 45, language);
        ...
        Activate_Inactivate_Pref(userNamePerformance, "PREF_POSITION_LEVEL_PERFORMANCE", 1, vServerPerformance);
        ...
        // Bouton segement
        StopWatchObj.Start();
        Get_PortfolioBar_BtnPerformance().WaitProperty("IsChecked", true, waitTimeShort);
        StopWatchObj.Stop();
        //***********************************************************************
        ...
        Activate_Inactivate_Pref(userNamePerformance, "PREF_POSITION_LEVEL_PERFORMANCE", null, vServerPerformance);
        RestartServices(vServerPerformance);
...
{
    var query = "select * from B_USER where STATION_ID = 'deslauje'";
	  
    var Qry = ADO.CreateADOQuery();
    Qry.ConnectionString = GetDBAConnectionString(vServerPerformance);
	  
    Qry.SQL=query;
    Qry.ExecSQL();//   .Open();
    Qry.First();
    var value = Qry.FieldByName("CONFIG_TXT").Value;
    Qry.Close();
