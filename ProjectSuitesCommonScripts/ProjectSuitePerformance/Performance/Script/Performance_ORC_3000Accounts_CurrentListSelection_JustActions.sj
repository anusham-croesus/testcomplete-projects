...
function Performance_ORC_3000Accounts_CurrentListSelection_JustActions()
{
        var cmbTransactionJustActions = ReadDataFromExcelByRowIDColumnID(filePath_Performance, "DataPerformanceNFR", "cmbTransactionUnitsPerAccount", language+client);        
        var SoughtForValueCurrentListSelectionJustActions = ReadDataFromExcelByRowIDColumnID(filePath_Performance, "DataPerformanceNFR", "SoughtForValueCurrentListSelectionJustActions", language+client);        
        
        Performance_ORC_3000Accounts(cmbTransactionJustActions, SoughtForValueCurrentListSelectionJustActions);
...
//        var cmbTransactionSellORC3000 = ReadDataFromExcelByRowIDColumnID(filePath_Performance, "DataPerformanceNFR", "cmbTransactionSellORC3000", language+client);        
        ...
        var symbolMSFT = ReadDataFromExcelByRowIDColumnID(filePath_Performance, "DataPerformanceNFR", "symbolMSFT", language+client);        
        var symbolAAPL = ReadDataFromExcelByRowIDColumnID(filePath_Performance, "DataPerformanceNFR", "symbolAAPL", language+client);        
        var qtyAAPL = ReadDataFromExcelByRowIDColumnID(filePath_Performance, "DataPerformanceNFR", "qtyAAPL", language+client);        
...
//************************** Étape 3 *********************************************************************************
        Log.PopLogFolder();
        logEtape3 = Log.AppendFolder("Étape 3: Ajouter transactions équivalentes (Achat) (50% (MSFT) et 5% (AAPL))");
        
        PurchasequivalentTransactAddiction(qtyBuyORC3000, symbolMSFT);  //Répartition: 50% (MSFT) et 
        PurchasequivalentTransactAddiction(qtyAAPL, symbolAAPL);   //Répartition: 5% (AAPL)
