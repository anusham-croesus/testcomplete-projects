...
function Performance_RCM_SwitchSecurityAlertsToReportsTab(){

        var StopWatchObj = HISUtils.StopWatch;
        var SoughtForValue = "Performance_RCM_SwitchSecurityAlertsToReportsTab";
        ...
        try {
          ...
          //Attendre l'onglet 'Reports' actif
          WaitObject(Get_CroesusApp(), "Uid", "TabItem_3a3b", waitTime);
          Get_WinRQS_TabReports().WaitProperty("Enabled", true, waitTime)                           
                     
          // Mesurer la performance de l'onglet 'Reports'       
          StopWatchObj.Start();
          Get_WinRQS_TabReports().Click(); 
          Get_WinRQS_TabReports().WaitProperty("IsSelected", true, waitTimeShort);
