...
function Performance_Por_ChangeDate() {

        var StopWatchObj = HISUtils.StopWatch;
        var SoughtForValue = "Performance_Por_ChangeDate";
...
//        var year = GetData(filePath_Performance, sheetName_DataBD, 7, language); 
//        var month = GetData(filePath_Performance, sheetName_DataBD, 8, language);
//        var day = GetData(filePath_Performance, sheetName_DataBD, 9, language);
...
//        var waitTimeShort = GetData(filePath_Performance, sheetName_DataBD, 45, language);
        ...
        var year       = ReadDataFromExcelByRowIDColumnID(filePath_Performance, sheetName_DataBD, "Year", language+client);
        var month      = ReadDataFromExcelByRowIDColumnID(filePath_Performance, sheetName_DataBD, "Month06", language+client);
        var day        = ReadDataFromExcelByRowIDColumnID(filePath_Performance, sheetName_DataBD, "Day19", language+client);
...
//        }
        ...
        Get_PortfolioGrid_BarToolBarTray_dtpDate().WaitProperty("Enabled", true, 15000);
        Get_PortfolioGrid_BarToolBarTray_dtpDate().Click(Get_PortfolioGrid_BarToolBarTray_dtpDate().get_ActualWidth() - 17, 13);
        Get_Calendar_LstYears_Item(year).Click();
        Get_Calendar_LstMonths_Item(month).Click();
        Get_Calendar_LstDays_Item(day).Click();
    
        // Mesure la performance modife la date  
        StopWatchObj.Start();
        Get_Calendar_BtnOK().Click();
        WaitObject(Get_PortfolioPlugin(), "Uid", "TextBlock_ce68", waitTimeShort);
        Get_PortfolioPlugin().FindChild("Uid", "TextBlock_ce68", 10).WaitProperty("VisibleOnScreen", true, 15000);
        StopWatchObj.Stop();
