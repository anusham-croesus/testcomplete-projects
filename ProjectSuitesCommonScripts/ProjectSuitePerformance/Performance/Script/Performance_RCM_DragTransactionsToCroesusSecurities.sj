...
function Performance_RCM_DragTransactionsToCroesusSecurities(){


//        DragTransactionsToCroesusModule(Get_ModulesBar_BtnSecurities());
//}
//
//function DragTransactionsToCroesusModule(moduleCible){


        var StopWatchObj = HISUtils.StopWatch;
        var SoughtForValue = "Performance_RCM_DragTransactionsToCroesusSecurities";
        ...
        try {
          ...
          Log.Message("Client number : "+ clientNumber);
                   
          // Mesurer la performance du maillage     
          StopWatchObj.Start();
          //Mailler vers 'Titres'
          Drag(Get_WinRQS_TabTransactionBlotter_BlotterControl().WPFObject("_transactionList").WPFObject("RecordListControl", "", 1).WPFObject("DataRecordPresenter", "", 1).FindChild("Value",clientNumber,10), Get_ModulesBar_BtnSecurities());
          //WaitObject(Get_CroesusApp(), "Uid", "DataGrid_865b", waitTimeShort);
          //Get_SecurityGrid().FindChild(["ClrClassName", "WPFControlOrdinalNo"], ["DataRecordPresenter", "1"], 10).WaitProperty("VisibleOnScreen", true, waitTime);
