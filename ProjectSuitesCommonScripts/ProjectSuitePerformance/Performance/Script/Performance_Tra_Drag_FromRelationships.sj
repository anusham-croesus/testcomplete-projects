...
function Performance_Tra_Drag_FromRelationships() {

            var StopWatchObj = HISUtils.StopWatch;
            var SoughtForValue = "Performance_Tra_Drag_FromRelationships";
...
//    
//                // Maillage une relations au module Transactions
//                StopWatchObj.Start();
//                Drag(Get_RelationshipsClientsAccountsGrid().FindChild(["ClrClassName", "WPFControlOrdinalNo"], ["DataRecordPresenter", posRelation], 10), Get_ModulesBar_BtnTransactions());
...
//            } else if (DataType == "Data"){
                ...
                Get_RelationshipsClientsAccountsGrid().Find("Value", relationshipName, 100).Click();
                Get_RelationshipsClientsAccountsGrid().FindChild(["ClrClassName", "WPFControlOrdinalNo"], ["DataRecordPresenter", "1"], 10).WaitProperty("IsInitialized", true, 15000);
            
                // Maillage une relations au module Transactions
                StopWatchObj.Start();
                Drag(Get_RelationshipsClientsAccountsGrid().Find("Value", relationshipName, 100), Get_ModulesBar_BtnTransactions());
                WaitObject(Get_CroesusApp(), "Uid", "FixedColumnListView_1b3e", waitTimeShort);
                Get_Transactions_ListView().FindChild(["ClrClassName", "WPFControlOrdinalNo"], ["DragableListViewItem", "2"], 10).WaitProperty("VisibleOnScreen", true, 15000);
                StopWatchObj.Stop();
                //*******************************************************************            
