...
function CR2180_Validate_matched_checkbox_in_transaction_info()
{
    try {
        /*Variables*/
        var previousTransactionsLastNumber = null;
        ...
        var myUsername = "christophep";//var myUsername = "philippema";
        var CRFolder = "CR2180";
        var filterName = "TEST_CR2180" + "_" + Math.round(Math.random()*200);
        ...
        previousTransactionsLastNumber = Execute_SQLQuery_GetField("select max(NO_TRANS) as NO_TRANS_MAX from B_TRANS", vServerTransactions, "NO_TRANS_MAX");
        ...
        if (previousTransactionsLastNumber !== null){
            Execute_SQLQuery("delete from B_TRANS where NO_TRANS > " + previousTransactionsLastNumber, vServerTransactions);
        }
...
{  
    ...
    Reprocess(CRFolder, myUsername);//Reprocess(myUsername);
}
...
function TestChris1()
{
    return CR2180_Validate_matched_checkbox_in_transaction_info();
